<?php 
            require '../../ConnectDB/connectDB.php';
                $id = $_GET['id'];
                $sqlck = "select * from product where cated_id='$id';";
                $resultck = mysqli_query($link, $sqlck);
               
                if(mysqli_num_rows($resultck) > 0){
                    echo"<script>";
                    echo"window.location.href='categorydetail.php?del=not';";
                    echo"</script>";
                }
               else{
                    $sqldel = "delete from categorydetail where cated_id='$id';";
                    $resultdel = mysqli_query($link, $sqldel);
                    if(!$resultdel)
                    {
                        echo"<script>";
                        echo"window.location.href='categorydetail.php?del=found';";
                        echo"</script>";
                    }
                    else{
                        echo"<script>";
                        echo"window.location.href='categorydetail.php?del=success';";
                        echo"</script>";
                    } 
               }
?>
